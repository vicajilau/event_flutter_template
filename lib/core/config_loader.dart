...
class ConfigLoader {
  ...
  static const String _githubUser = String.fromEnvironment('GITHUB_USER', defaultValue: 'vicajilau');
  static const String _githubRepo = String.fromEnvironment('GITHUB_REPO', defaultValue: 'event_flutter_template');
  static const String _githubBranch = String.fromEnvironment('GITHUB_BRANCH', defaultValue: 'main');
  ...
  static Future<SiteConfig> loadConfig() async {
    ...
    switch (_appEnv) {
      case 'pro':
        ...
        final configUrl = '$baseUrl/$year/config/site.json';
        final res = await http.get(Uri.parse(configUrl));
        if (res.statusCode != 200) {
          throw Exception("Error cargando configuración de producción desde $configUrl");
        }
        configContent = res.body;
        break;
      case 'pre':
        // Entorno de preproducción: Carga desde el raw de GitHub (rama específica).
        baseUrl = 'https://raw.githubusercontent.com/$_githubUser/$_githubRepo/$_githubBranch';
        final configUrl = '$baseUrl/$year/config/site.json';
        final res = await http.get(Uri.parse(configUrl));
        if (res.statusCode != 200) {
          throw Exception("Error cargando configuración de pre-producción desde $configUrl");
        }
        configContent = res.body;
        break;
      case 'dev':
      default:
        ...
        final configPath = '2025/config/site.json';
        try {
            configContent = await rootBundle.loadString(configPath);
            // En dev, la baseUrl es la ruta base de los assets.
            baseUrl = '2025'; 
        } catch (e) {
            throw Exception("Error cargando config local: $configPath. Asegúrate de que esté en pubspec.yaml");
        }
        break;
    }
